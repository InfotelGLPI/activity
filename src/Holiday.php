...
class Holiday extends CommonDBTM
{
    ...
    * */
    ...
    function defineTabs($options = [])
    {
        ...
        $this->addStandardTab(HolidayValidation::class, $ong, $options);
    ...
    **/
    function prepareInputForAdd($input)
    {
        ...
        if (!isset($input["plugin_activity_holidaytypes_id"])
          || $input["plugin_activity_holidaytypes_id"] == 0) {
            ...
            if ($holidayType->fields['auto_validated'] == 1) {
                $input['global_validation'] = CommonValidation::ACCEPTED;
            }
        }
        ...
        if (!isset($input['global_validation'])) {
            $input['global_validation'] = CommonValidation::NONE;
        }
    ...
    function post_addItem()
    {
        ...
        if (sizeof($datas) > 0) {
            foreach ($datas as $data) {
                ...
                $holidayValidation->fields['status']                      = CommonValidation::WAITING;

                if ($this->fields['global_validation'] == CommonValidation::ACCEPTED) {
                    $holidayValidation->fields['status']             = CommonValidation::ACCEPTED;
    ...
    private function prepareDates($input)
    {
       ...
       // Same date
        if ($input["begin"] == $input["begin"]) {
            ...
            $input['end'] = $he . ' ' . Report::getPmEnd();
        }

        switch ($hlfDayEnd) {
            case Report::$AM_LABEL:
                $input["end"] = $he . ' ' . Report::$AM_END;
                break;
            case Report::$PM_LABEL:
            case Report::$ALL_DAY_LABEL:
                $input["end"] = $he . ' ' . Report::getPmEnd();
                break;
        }
        ...
        switch ($hlfDayBegin) {
            ...
            case Report::$AM_LABEL:
                ...
                break;
            case Report::$PM_LABEL:
                $input["begin"] = $hb . ' ' . Report::$PM_BEGIN;
                break;
        }
        return $input;
    }
    ...
    static function getActionsOn()
    {
        ...
        $opt['criteria'][0]['value']      = CommonValidation::WAITING;
    ...
    static function getSpecificValueToSelect($field, $name = '', $values = '', array $options = [])
    {
        ...
        switch ($field) {
            case 'global_validation':
                ...
                $options['id']    = '';
                return CommonValidation::dropdownStatus($name, $options);
    ...
    */
    ...
    public function getPeriodForTemplate($at)
    {
       ...
       // default values
        ...
        if ($actionTime > 0) {
            ...
            if ($momentDeb == Report::getAmBegin() && $momentEnd == Report::getPmEnd() && !$isSameDate) {
    ...
    private function getCbsForPeriod($at)
    {
       ...
       // default values
        ...
        $arrayRet['cbEnd'][Report::$AM_LABEL]['disabled']        = true;
        $arrayRet['cbEnd'][Report::$PM_LABEL]['disabled']        = true;
        $arrayRet['cbEnd'][Report::$ALL_DAY_LABEL]['disabled']   = true;
        ...
        if ($actionTime > 0) {
            ...
            if ($momentDeb == Report::getAmBegin() && $momentEnd == Report::getPmEnd() && !$isSameDate) {
                ...
                $arrayRet['cbEnd'][Report::$ALL_DAY_LABEL]['disabled']   = false;
                $cbBeginChecked                                                        = Report::$ALL_DAY_LABEL;
        ...
        if (isset($this->fields['id']) && $this->fields['id'] > 0) {
            $arrayRet['cbBegin'][Report::$AM_LABEL]['disabled']      = true;
            $arrayRet['cbBegin'][Report::$PM_LABEL]['disabled']      = true;
            $arrayRet['cbBegin'][Report::$ALL_DAY_LABEL]['disabled'] = true;
            $arrayRet['cbEnd'][Report::$AM_LABEL]['disabled']        = true;
            $arrayRet['cbEnd'][Report::$PM_LABEL]['disabled']        = true;
            $arrayRet['cbEnd'][Report::$ALL_DAY_LABEL]['disabled']   = true;
        }
    ...
    * */
    function showForm($ID, $options = [])
    {
        ...
        if (isset($this->fields["begin"]) && !empty($this->fields["begin"])) {
            ...
            $begin = date("Y-m-d") . " " . Report::getAmBegin();
    ...
    function getBodyMail($dateComplete, $date, $userName, $approverFullName)
    {
        ...
        foreach ($rows as $row => $data) {
           //get row data
            ...
            } elseif (strpos($data, "{{user_name}}") !== false) {
                $finalRows .= str_ireplace("{{user_name}}", $userName, $data);
            } elseif (strpos($data, "{{holiday_date}}") !== false) {
...
//                    ;");
    ...
    */
    function countWe($debut, $fin, $holidays = [])
    {
        ...
        for ($i = $start; $i <= $end; $i = $i + 86400) {
            ...
            $isholiday = Holiday::checkInHolidays($input, $holidays);
    ...
    */
    function excludeWe($debut, $fin, &$interv, $key, $id)
    {
        ...
        for ($i = $start; $i <= $end; $i = $i + 86400) {
            ...
            $isholiday = Holiday::checkInHolidays($input);

            if (in_array($joursem[$jour], $We) || $isholiday) {
                if ($begin) {
                    $interv[$new_key]['end'] = date('Y-m-d', ($i - 86400)) . ' ' . Report::getPmEnd();
                    $begin                   = false;
                }
            } else {
                if (!$begin) {
                    $data_begin                = date('Y-m-d', $i) . ' ' . Report::getAmBegin();
                    $new_key                   = $data_begin . "$$" . "GlpiPlugin\Activity\Holiday$id";
    ...
    */
    function getSpecificMassiveActions($checkitem = null)
    {
        ...
        if ($isadmin) {
            $actions['GlpiPlugin\Activity\Holiday' . MassiveAction::CLASS_ACTION_SEPARATOR . 'updateAllValidations'] = _n('Approve holiday', 'Approve holidays', 2, 'activity');
        }
