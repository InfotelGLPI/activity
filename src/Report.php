    ...
    function showCRA($input, $output_type, CraPDF $PDF, $pdfMode = false)
    {
       ...
       // 1.1 Plugin Activity
        if ($use_planning_activity_hours) {
            ...
            $externalEvents = PlanningExternalEvent::getExternalEventWithActivityOnPeriod(
                $crit['begin'],
                $crit['end'],
                $crit['users_id'],
                true,
                ...
                if ($DB->numrows($resulth)) {
                    while ($datah = $DB->fetchArray($resulth)) {
                      ...
                      // Repartition of the time
                        ...
                        $this->item_search[$type][Holiday::class][date('Y-m-d', strtotime($datah["begin"]))][] = $datah['id'];
                    }
                }
            }
            ...
            if ($numbert != "0") {
                if ($use_planning_activity_hours) {
                    ...
                    foreach ($ticketTasks as $ticketTask) {
                        ...
                        $internal = Config::getConfigFromDB($ticketTask['entities_id']);
                        if ($internal) {
                            foreach ($internal as $field) {
                ...
                foreach ($data as $name => $day) {
                    ...
                    foreach ($days as $k => $v) {
                        ...
                        //                     if (array_key_exists($morning, $day)) {
                            ...
                            } elseif (!isset($activities[$type][$name][$count][$v])) {
                     ...
                     INNER JOIN `glpi_plugin_activity_planningexternalevents`
                        ...
                        if ($numbert != "0") {
                            while ($datat = $DB->fetchArray($resultt)) {
                                $comment .= $datat["text"] . " (" . (self::TotalTpsPassesArrondis($use_hour_on_cra ? $datat["actiontime"] : ($datat["actiontime"] / $AllDay), $options)) . ")<br>";
                            }
                        }
    ...
    static function setTimes($time, $hour, $date_add, $values, $type, $activity, $holidays = [], $timejour, $allDay, $options)
    {
        ...
        if (!$isWe) {
            if (!empty($values[$type][$activity])) {
                ...
                } elseif (isset($values[$type][$activity][$date_add . ' ' . $hour])) {
