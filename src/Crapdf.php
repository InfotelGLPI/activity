...
class CraPDF extends Fpdf
{
    ...
    public $timeHeader          = [];
    public $holiday             = [];
    public $holidayHeader       = [];
    public $holidayComments     = [];
    public $generalinformations = [];
    public $footerInformations  = [];
    public $total_all           = 0;
    public $no_cra              = ""; // Num du document, généré.
    public $working_days        = 0;
    public $all_days            = 0;

    /* Constantes pour paramétrer certaines données. */
    public $line_height               = 3.5;     // Hauteur d'une ligne simple.
    public $day_width                 = 6;       // Largeur de cellule jour
     ...
     */
    public function setGeneralInformations($generalinformations)
    {
    ...
    public function drawGeneralInformations()
    {
    ...
    public function setTimeHeader($timeHeader)
    {
    ...
    public function getTimeHeader()
    {
        return $this->timeHeader;
    }

    public function drawTimeHeader()
    {
     ...
     */
    public function computeTotal($data_time = [])
    {
        $total_all = 0;
        foreach ($data_time as $times) {
            foreach ($times as $data) {
                $total_all += Report::TotalTpsPassesArrondis($data['values']);
            }
        }
     ...
     * @param       $type
     */
    public function drawTime($type, $data_time = [])
    {
        ...
        foreach ($data_time as $activity => $times) {
            ...
            foreach ($times as $data) {
                ...
                if (isset($data['options']['weekend'])) {
                    $color = 'hardgrey';
                } elseif (isset($data['options']['depass'])) {
                    $color      = $line_color;
                    $font_color = 'red';
                } else {
                    $color = $line_color;
                ...
                // Use round values
                $data['values'] = Report::TotalTpsPassesArrondis($data['values']);
            ...
            if (Report::isIncorrectValue($total_activity)) {
                $font_color = 'red';
            }
    ...
    public function completeTable($all_days, $h, $border)
    {
        if ($all_days < 31) {
     ...
     */
    public function setHoliday($holiday = [], $sickness = [], $part_time = [])
    {
        if (empty($part_time)) {
    ...
    public function setHolidayHeader($holidayHeader)
    {
        $this->holidayHeader = $holidayHeader;
    }

    public function setHolidayComments($holidayComments)
    {
    ...
    public function getHolidayHeader()
    {
    ...
    public function drawHolidayHeader()
    {
    ...
    public function setAllDays($all_days)
    {
    ...
    public function setFooterInformations($footerInformations)
    {
    ...
    public function DrawCra()
    {
        ...
        if (!empty($times)) {
            $this->drawTimeHeader();
            $this->drawTime(Report::$WORK, $times);
        }
        ...
        if (!empty($holidays)) {
            $this->drawTime(Report::$HOLIDAY, $holidays);
        }
     ...
     */
    public function GetNoCra($une_date = "")
    {
