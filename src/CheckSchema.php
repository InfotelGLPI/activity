...
class CheckSchema extends CommonDBTM
{
    ...
    {
        return __('Schema check', 'activity');
    }
    ...
    {
        return "glpi_plugin_activity_options";
    }
    ...
    public static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0)
    {
        if ($item->getType() == Option::class) {
            $self = new self();
            $self->checkSchema(PLUGIN_ACTIVITY_VERSION);
        }
        return true;
    }
    ...
    public function getSchemaPath(?string $version = null): ?string
    {
        ...
        return Plugin::getPhpDir('activity') . "/install/sql/empty.sql";
    }
    ...
    public function checkSchema(
        ...
        try {
            ...
            $differences = $checker->checkCompleteSchema($schemaFile, true, 'plugin:activity');
            ...
            TemplateRenderer::getInstance()->display('@activity/checkschema_result.html.twig', [
                'error'           => __('Failed to check the sanity of the tables!', 'activity'),
                'results'         => [],
                'all_safe'        => [],
                'all_destructive' => [],
            ]);
