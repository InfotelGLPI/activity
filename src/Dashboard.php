...
//               $datas[] = $times;
            ...
            //                    Form to choose user, and then see user's planning, toggleRefresh to enable the automatic refresh
                ...
                break;
            case $this->getType() . "3":
                $widgetHTML = new MydashboardHtml();
                $widgetHTML->setWidgetTitle(__("Activity Menu", 'activity'));
                $listActions = array_merge(PlanningExternalEvent::getActionsOn(), PlanningExternalEvent::getActionsOn());
                $widgetHTML->setWidgetHtmlContent(PlanningExternalEvent::menu(PlanningExternalEvent::class, $listActions, true));
                return $widgetHTML;
                break;
            case $this->getType() . "4":
...
//                  }
         ...
         //         # 1.3 Tickets
            if ($numbert != "0") {
                ...
                foreach ($sums as $k => $cnt) {
                    $sum += Report::TotalTpsPassesArrondis($cnt);
                }
                $pie[$mtitle] = $sum;
            }
    ...
    public function getActivities($users_id, $activities_id = 0)
    {
        ...
        $default_view = PlanningExternalEvent::$DAY;
            ...
            foreach ($values as $k => $v) {
                foreach ($v as $id => $data) {
                    ...
                    foreach ($data as $date => $duration) {
                        ...
                        if ($use_pairs == 1) {
                            $end = date("Y-m-d H:i:s", strtotime($begin) + (Report::TotalTpsPassesArrondis($duration) * $AllDay));
                        } else {
        ...
        if ($DB->numrows($resulth)) {
            ...
            foreach ($values as $k => $v) {
                foreach ($v as $id => $data) {
                    foreach ($data as $date => $duration) {
                        $action = Report::TotalTpsPassesArrondis($duration) * $AllDay;
