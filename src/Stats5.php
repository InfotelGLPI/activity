...
class Stats5 implements ActivityInterface {
   ...
   public function getData(&$params) {
      ...
      $result_gu = $DB->fetchArray($DB->doQuery($query_group_member));
      $techlist = explode(',', $result_gu['listuser']);

      $nb_ticket = [];
      $nb_tech = [];
      $entitiesRestrict['glpi_tickets'] = Tools::getSpecificEntityRestrict('glpi_tickets', $params);
      $entitiesRestrict['glpi_plugin_mydashboard_stocktickets'] = Tools::getSpecificEntityRestrict('glpi_plugin_mydashboard_stocktickets', $params);
      $entitiesRestrict['glpi_profiles_users'] = Tools::getSpecificEntityRestrict('glpi_profiles_users', $params);
      $entitiesRestrict['glpi_plugin_activity_activities'] = Tools::getSpecificEntityRestrict('glpi_plugin_activity_activities', $params);

      $this->months = Tools::getMonths($params);
      $params['maxyaxis'] = 0;
      foreach ($this->months as $key => $month) {
         ...
         /*if(isset($params['nb_ticket']) && $params['nb_ticket'] == 1){
            ...
            $result_t = $DB->fetchArray($DB->doQuery($querym_t));
         ...
         if (isset($params['nb_ticket']) && $params['nb_ticket'] == 1) {
            ...
            if ($key == date("m") && $year == date("Y")) {
               ...
               $result_t2 = $DB->fetchArray($DB->doQuery($querym_t2));
         ...
         if (isset($params['nb_tech']) && $params['nb_tech'] == 1) {
            ...
            foreach ($techlist as $techid) {
               ...
               $result_at_q = $DB->doQuery($querym_at);

               while ($data = $DB->fetchAssoc($result_at_q)) {
                  $tot_time[$key] += (Report::TotalTpsPassesArrondis($data['actiontime']/3600/8))/$nb_worked_days;
               }
               ...
               while ($data = $DB->fetchAssoc($result_ai_q)) {
                  $tot_time/*[$techid]*/[$key] += (Report::TotalTpsPassesArrondis($data['actiontime_date']/3600/8))/$nb_worked_days;
               }
            }
            //      }
         }
